...
# 执行自定义训练过程
def mnist_fnn_cor_attack_train(model, optimizer, gama):
    ...
    for epoch in range(100):
        ...
        print('epoch:', epoch, 'Evaluate Acc:', float(acc), 'mape:', mape)
    data = recover_cor_stolen_data(model, x_train)
    test_accuracy_name = 'mnist_linear//cor//mnist_fnn_cor_attack' + '_test_accuracy_' + str(gama) + '_' + str(100)
    mape_name = 'mnist_linear//cor//mnist_fnn_cor_attack' + '_mape_' + str(gama) + '_' + str(100)
    data_name = 'mnist_linear//cor//mnist_fnn_cor_attack' + '_stolen_data_' + str(gama) + '_' + str(100)
